...
namespace KSU.CIS300.ConnectFour.Test
{
    public class GameUnitTests
    {
        ...
        {
            Assert.That(Game.ColumnSize == 6);
            Assert.That(Game.ColumnLabels == "ABCDEFG");
        ...
        public void A5_GameUnitTest_InitColumns()
        {
            ...
            char[] cols = Game.ColumnLabels.ToCharArray();
            Array.Reverse(cols);
            Assert.That(_g.Column != null);
        ...
        public void C5_GameUnitTest_PlaceNewPiece_StackLinked()
        {
            ...
            Assert.That(_g.PlaceNewPiece(Color.Red, "D", out row) == "D2");
            Assert.That(row == 2);
            assertCellHelper(_g.Column.Data.Previous, Color.Black, 1, 'D');
            assertCellHelper(_g.Column.Data.Previous.Next, Color.Red, 2, 'D');
            assertCellHelper(_g.Column.Data, Color.Red, 2, 'D');

            Assert.That(_g.PlaceNewPiece(Color.Black, "D", out row) == "D3");
            Assert.That(row == 3);
            assertCellHelper(_g.Column.Data.Previous.Previous, Color.Black, 1, 'D');
            assertCellHelper(_g.Column.Data.Previous.Previous.Next, Color.Red, 2, 'D');
            assertCellHelper(_g.Column.Data.Previous, Color.Red, 2, 'D');
            assertCellHelper(_g.Column.Data.Previous.Next, Color.Black, 3, 'D');
            assertCellHelper(_g.Column.Data, Color.Black, 3, 'D');
        }
        ...
        public void E1_GameUnitTest_CheckWin_Vertical()
        {
            int row=0;
            string id = "";
            foreach (char c in Game.ColumnLabels)
            {
                _g = new Game();
                for (int r = 0; r < Game.ColumnSize; r++)
                {
        ...
        public void E2_GameUnitTest_CheckWin_VerticalStack()
        {
            ...
            for (int r = 3; r <= Game.ColumnSize; r++)
            {
                id = _g.PlaceNewPiece(Color.Black, "A", out row);
            }
            DoubleLinkedListCell<GamePiece> found;
        ...
        public void E3_GameUnitTest_CheckWin_Horizontal()
        {
            int row=0;
            string id = "";
            for (int r = 1; r <= Game.ColumnSize; r++)
            {
                _g = new Game();
                

                foreach (char c in Game.ColumnLabels.Substring(0,4))
                {
                    id = _g.PlaceNewPiece(Color.Black, c.ToString(), out row);
                }
                DoubleLinkedListCell<GamePiece> found;
        ...
        public void E4_GameUnitTest_CheckWin_HorizontalLoss()
        {
            int row;
            string id = "";
            foreach (char c in Game.ColumnLabels.Substring(0, 3))
            {
                id = _g.PlaceNewPiece(Color.Black, c.ToString(), out row);
            }
        ...
        public void E6_GameUnitTest_CheckWin_DiagnalDownRight()
        {
            ...
            foreach (char c in Game.ColumnLabels.Substring(0,4))
            {                
                for (int r = i; r < Game.ColumnSize; r++)
                {
                    id = _g.PlaceNewPiece(Color.Black, c.ToString(), out row);
                }
        ...
        public void E10_GameUnitTest_CheckWin_DiagnalUpRight()
        {
            int row;
            string id = "";
            int i = Game.ColumnSize;
            foreach (char c in Game.ColumnLabels.Substring(2, 4))
            {
                for (int r = i; r > 1; r--)
                {
                    id = _g.PlaceNewPiece(Color.Black, c.ToString(), out row);
        ...
        public void E12_GameUnitTest_CheckWin_DiagnalUpRightMiddle()
        {
            ...
            string id = "";
            int i = Game.ColumnSize;
            foreach (char c in Game.ColumnLabels.Substring(3, 4))
            {
                for (int r = i; r > 1; r--)
                {
                    id = _g.PlaceNewPiece(Color.Black, c.ToString(), out row);
